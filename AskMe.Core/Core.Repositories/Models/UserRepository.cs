...
namespace AthenicConsulting.Core.Core.Models.Repositories
{
    public class UserRepository : IUserRepository
    {
        ...
        public UserRepository(AskMeDbContext context, UserManager<ApplicationUser> userManager, SignInManager<ApplicationUser> signInManager, IAskMeSettings askMeSettings, ILogger<UserRepository> logger)
        {
            _context = context;
            _userManager = userManager;
            _signInManager = signInManager;
            _askMeSettings = _askMeSettings;
            _logger = logger;
        }
        ...
        public async Task<string> GetResetPasswordCallbackUrl(ForgotPasswordViewModel forgotPasswordViewModel)
        {
            ...
            if (user != null)
            {
                ...
                var callbackUrl = $"{_askMeSettings.AskMeApiUrl}/Account/ResetPassword?UserId={user.Id}&Code={encoded}";
                return callbackUrl;
        ...
        public async Task<string> GetAccountConfirmationLink(string email)
        {
            ...
            try
            {
                ...
                if (user != null)
                {
                    ...
                    link = $"{_askMeSettings.AskMeApiUrl}/Account/ConfirmAccount?UserId={user.Id}&Code={token}";
                }
            }
            catch (Exception ex)
            {
